...
void print_types(Window window) {
	...
	if (type == netdesktop) {
		_log(" ____%s  __WINDOW_%d__ATYPE_DESKTOP ____\n", __FUNCTION__, window);
	} else if (type == netdock) {
		_log(" ____%s  __WINDOW_%d__ATYPE_DOCK ____\n", __FUNCTION__, window);
	} else if (type == nettoolbar) {
		_log(" ____%s  __WINDOW_%d__ATYPE_TOOLBAR ____\n", __FUNCTION__, window);
	} else if (type == netmenu) {
		_log(" ____%s  __WINDOW_%d__ATYPE_MENU ____\n", __FUNCTION__, window);
	} else if (type == netutility) {
		_log(" ____%s  __WINDOW_%d__ATYPE_UTILITY ____\n", __FUNCTION__, window);
	} else if (type == netsplash) {
		_log(" ____%s  __WINDOW_%d__ATYPE_SPLASH ____\n", __FUNCTION__, window);
	} else if (type == netdialog) {
		_log(" ____%s  __WINDOW_%d__ATYPE_DIALOG ____\n", __FUNCTION__, window);
	} else if (type == netnormal) {
		_log(" ____%s  __WINDOW_%d__ATYPE_NORMAL ____\n", __FUNCTION__, window);
	} else {
		_log(" ____%s  __WINDOW_%d__ATYPE_UNKNOWN ____\n", __FUNCTION__, window);
	}
}
...
void init_prog() {
	...
	_log(" ____%s  __ROOT_WINDOW_INIT_%d___MONITOR_%d ____\n", __FUNCTION__, last->window, last->monitor);
...
program * add_prog(Window win) {
	_log(" ____%s  __WINDOW_%d ____\n", __FUNCTION__, win);

	if (win == root || win == rootw || win == 1) {
		_log(" ____%s  __IMPOSTOR WINDOW_%d ____\n", __FUNCTION__, win);
		return NULL;
	}
	...
	for (;;) {
		prog = prog->next;
		if (prog->window == win) {
			_log(" ____%s  __WINDOW_%d___EXIST ____\n", __FUNCTION__, win);
			exist = true;
			obj = prog;
		}
		if (prog == first) break;
...
program * get_prog(Window win) {
	...
	for (;;) { //for (int p = 0; p <= prog_count; p++) {
		prog = prog->next;
		if (prog->window == win) {
			_log(" ____%s  __WINDOW_%d___FOUND ____\n", __FUNCTION__, win);
			return prog;
		}
		if (prog == first) break;
	}
...
//	if (win == 1) return first;
	...
	_log(" ____%s  __WINDOW_%d___DOES_NOT_EXIST ____\n", __FUNCTION__, win);
...
Window under_pointer() {
	...
	_log(" ____%s  __WINDOW_UNDER_POINTER_%d ____\n", __FUNCTION__, ret);
	return ret;
}
...
void init_mon() {
	...
	_log(" ____%s  __ROOT_MONITOR_INIT_%d___ID_%d ____\n", __FUNCTION__, last_mon, last_mon->id);
}
...
void add_mon(int numb) {
	...
	_log(" ____%s  __MONITOR_%d_ADDED ____\n", __FUNCTION__, numb);
}


monitor *get_mon(int numb) {
	_log(" ____%s  __MONITOR_%d ____\n", __FUNCTION__, numb);
...
void rem_prog(Window win) {
	...
	for (;;) { //for (int i = 0; i <= prog_count; i++) {
		if (i > max_stack) {
			_log(" ____%s  __MAX_STACK___WINDOW_%d ____\n", __FUNCTION__, prog->window);
			break;
		}
		i++;

		prog = prog->next;
		_log(" ____%s  __REMOVE_ITERATION___WINDOW_%d ____\n", __FUNCTION__, prog->window);

		if (prog->window == win) {
			_log(" ____%s  __WINDOW_%d___REMOVING___NEXT_%d___PREV_%d___LAST_%d  _\n", __FUNCTION__, win, prog->next->window, prog->prev->window, last->window);
...
//			last = prog != first ? prog->next : prog->prev;
			...
			_log(" ____%s  __WINDOW_%d___REMOVED___ID_%d ____\n", __FUNCTION__, win, prog->id);
			break;
...
void restack_mon(int numb) {
	_log(" ____%s  __CURR_MON_%d ____\n", __FUNCTION__, curr_mon);
...
void change_mon(int numb, Window focused, int _mask) {
	...
	if (_mask == (KeyMask | ControlMask) ) {
		...
		if (prog->monitor != numb) {
			prog->monitor = numb;
			_log(" ____%s  __WINDOW_%d___NOW_MONITOR_%d CURR_MON_%d ____\n", __FUNCTION__, prog->window, prog->monitor, curr_mon);
		}
	...
	if (tmpmons != NULL && tmpprog != NULL && tmpprog->monitor != curr_mon) {
		...
		for (;;) {
			...
			if (tmp->window != root && tmp->monitor == curr_mon) {
				...
				_log(" ____%s  __CURRMON_%d___MONITOR_%d___LASTFOCUSED_%d ____\n", __FUNCTION__, curr_mon, tmpmons->id, tmpmons->last_focused);
...
//	_log(" ____%s  _WINDOW_FOCUSED_%d ____\n", __FUNCTION__, winfoc);
	return winfoc;
}
...
void change_program(Window win, int mon) {
	...
	for (;;) {//for (int i = 0; i <= prog_count; i++) {
		...
		prog = prog->next;
		_log(" ____%s  __PROG_%d___WINDOW_%d ____\n", __FUNCTION__, prog->id, prog->window);
...
void counter() {
	...
	for (;;) {
		prog = prog->next;
	//	_log(" ____%s  __COUNT___WINDOW_%d___ID_%d ____\n", __FUNCTION__, prog->window, prog->id);

		_log("			 ____%s  __COUNT___WINDOW_%d___ID_%d___NEXT_%d___PREV_%d___LAST_%d___MONITOR_%d___PROG_STACKED_%b  _\n", __FUNCTION__, prog->window, prog->id, prog->next->window, prog->prev->window, last->window, prog->monitor, prog->stacked);

		count++;
		if (prog == first) break;

	}
	_log("			 ____%s  __COUNT_%d ____\n", __FUNCTION__, count);


	_log("			 ____%s  __FOCUSED_%d ____\n", __FUNCTION__, focused);
...
void switch_program(Window win) {
	...
	_log(" ____%s  __SWITCHED_%d_ID_%d___TO_%d_ID_%d ____\n", __FUNCTION__, prog1->window, prog1->id, prog2->window, prog2->id);
...
void stack_programs(int mon, bool alls) {
	...
	for (;;) { //for (int i = 0; i <= prog_count + 10; i++) {
		...
		if (prog->monitor == mon && (!alls ? prog->stacked == true : true) ) {
			prog->stacked = true;
			_log(" ____%s  _____CURRENT_%d___ROOT_%d ____\n", __FUNCTION__, prog->window, root);
			count++;
		}
		if (prog == first) break;
	...
	_log(" ____%s  _____PROGS_ALL_%d___PROGS_AT_SCREEN_%d ____\n", __FUNCTION__, progs, count);
	...
	for (;;) {
		...
		if (prog->window != root && prog->monitor == curr_mon && (!alls ? prog->stacked == true : true) ) {
			...
			_log(" ____%s  _____WINDOW_%d_STACKED_X_%d_Y_%d_WIDTH_%d_HEIGHT_%d ___\n", __FUNCTION__, prog->window, _x + _bw, _y + _bw, _sw - _bw * 2, _sh - _bw * 2);
...
void move_resize_window(XButtonEvent *e) {
	_log(" ____%s  _CALLER_%d ____\n", __FUNCTION__, e->window);

	Window win = under_mouse;//get_focused();
	_log(" ____%s  _FOCUSED_%d ____\n", __FUNCTION__, win);
	...
	_log(" ____%s  _C_%d___R_%d___RX_%d___RY_%d___CX_%d___CY_%d ____\n", __FUNCTION__, c, r, rx, ry, cx, cy);
...
void handle_expose(XExposeEvent *e) {
	...
	// if (e->window != root) XReparentWindow(display, e->window, root, 0, 0);
...
void handle_key_press(XKeyEvent *e) {
	...
	_log(" ____%s  __MASK_%d___MASKCTRL_%d ____\n", __FUNCTION__, _mask, ControlMask);
	_log(" ____%s  __MASKED_%d ____\n", __FUNCTION__, e->keycode);
...
//	XGrabKey(display, XKeysymToKeycode(display, XK_F4), KeyMask, root, True, GrabModeAsync, GrabModeAsync);
	...
	*/
			...
			case DestroyNotify:
				...
				break;
			default:
				_log(" ____%s  __UNKNOWN_EVENT_%d ____\n", __FUNCTION__, e.type);
				break;
		}
	}
}
...
#define catch else
#define perror(e) _log("Netsing level %d: error %dn", rs, e);
*/
