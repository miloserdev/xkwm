...
program * add_prog_with_check(Window win) {
	...
	program *obj = first;
	for(int i = 0; i < prog_count; i++) {
		printf("________FUNC_%s__________CHECKING_PROG_%d___WINDOW_%d_________\n", __FUNCTION__, obj->window, win);
	...
	if (!exist) {
		...
		printf("________FUNC_%s__________NEW_PROGRAM___WINDOW_%d_________\n", __FUNCTION__, win);

		monitor *mon = get_mon(prog->monitor);

		mon->count = mon->count + 1;

		mon->prevs = (mon->count > 0) ? prog->window : root;
...
//		if (prog->next != NULL) prog = prog->next;
		if ( get_prog(prog->window) && (int) prog->window != root) {
			if (prog->monitor != curr_monitor) {
...
//	XSetInputFocus(display, root, RevertToPointerRoot, CurrentTime);
	...
	printf("________FUNC_%s_____PROG_COUNT_%d___MONITOR_%d___LAST_WINDOW_%d___PREV_WINDOW_%d___WINFOCUS_%d___REV_FOCUS_%d___CURR_MONITOR_%d_____\n", __FUNCTION__,
								prog_count, mon->id, mon->lasts, mon->prevs, winfocus, revert_focus, curr_monitor);
...
void mkill(Window window) {
	printf("________FUNC_%s__________WINDOW_%d_________\n", __FUNCTION__, window);
...
void process_win(int _mask, Window winfocus, Window window, int revert_focus, int monnumb) {
	if (_mask == (Mod1Mask | ControlMask) ) {
		...
		mt->lasts = mt->prevs;
		mt->count = mt->count > 0 ? mt->count - 1 : 0;
		monitor *mn = get_mon(monnumb);
		mn->lasts = winfocus;
		mn->count += 1;

		XRaiseWindow(display, mn->lasts);
		XSetInputFocus(display, mn->lasts, RevertToPointerRoot, CurrentTime);

		change_monitor(curr_monitor);
		return;
//		break;
	} else {
		printf("__________CHANGING_MON___CURRENT_%d___NEW_%d______WINDOW_%d______WINFOCUS_%d______REVERT_FOCUS_%d____\n", curr_monitor, monnumb, window, winfocus, revert_focus);
...
*/
	...
	case XK_2:
		if (e->state & Mod1Mask) {
			process_win(_mask, winfocus, e->window, revert_focus, 2);
		}
		break;

	case XK_3:
		if (e->state & Mod1Mask) {
			process_win(_mask, winfocus, e->window, revert_focus, 3);
		}
		break;
...
//mbthis		XRaiseWindow(display, e->window);
//		XSetInputFocus(display, e->window, 0, CurrentTime);
//		XSetInputFocus(display, e->window, RevertToPointerRoot, CurrentTime);
		if (e->window != root) XSetInputFocus(display, e->window, RevertToPointerRoot, CurrentTime);
...
void handle_client_message(XClientMessageEvent *e) {
	printf("________FUNC_%s__________WINDOW_%d___TYPE_%d___ATOM_%d_________\n", __FUNCTION__, e->window, e->type, e->message_type);
